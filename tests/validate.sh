...
grep -Eq '^is_user_creation_disabled: false$' "$repo_root/stack.config/donetick/selfhosted.yaml" || {
  printf '[module-contract] Donetick must permit Keycloak first-login provisioning in selfhosted.yaml\n' >&2
  exit 1
}
grep -Eq '^      DT_IS_USER_CREATION_DISABLED: "false"$' "$repo_root/stack.runtime.yaml" || {
